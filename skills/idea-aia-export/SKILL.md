...
PS 版同样：`-SessionId <GUID>`、`-OutDir`。PS 导出也会在 md 头部加一行 `所属项目:`（本机命中前推断）。
...
```bash
# 按项目统计会话数（推荐看这个分布）
python scripts/export_aia_sessions.py --summary

# 全部会话列表里带项目列
python scripts/export_aia_sessions.py --list [--project 项目名子串] [--unknown] [--project-all]
```
